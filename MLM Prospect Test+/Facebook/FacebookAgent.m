...
//  Pagination
//
//  Created by Asad Khan on 1/6/10.
...
- (void)dialogDidSucceed:(FBDialog*)dialog{
	...
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Congratulations" message:@"Your catch has been shared on facebook" delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil];
	[alert show];
	[alert release];
}
...
- (void)uploadScreen:(UIImage *)screenshot
{   
	//[NSString stringWithFormat: @"%@", self.illusion]
	/*NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:self.illusion, @"status",@"true", @"status_includes_verb",nil];
	[[FBRequest requestWithDelegate:self] call:@"facebook.users.setStatus" params:params];
	loginButton.alpha=0;
	backButtonView.alpha=0;	
	shareButtonView.alpha=0;	
	backButton.alpha=0;	
	...
	NSLog(@"label1=%@",palabra1.text);
	NSLog(@"label1=%@",palabra2.text);
	NSLog(@"label1=%@",palabra3.text);
	NSLog(@"model=%@",self.model.firstpalabra);
	
	
	[[FBRequest requestWithDelegate:self] call:@"facebook.photos.upload" params:nil dataParam:(NSData*)screenshot];
	
	//[[FBRequest requestWithDelegate:self] call:@"facebook.users.setStatus" params:params];
	[self alertDone];
	backButtonView.alpha=1;	
	shareButtonView.alpha=1;	
	backButton.alpha=1;	
	...
	[[FBRequest requestWithSession:_session delegate:self] call:@"facebook.photos.upload" params:nil dataParam:self.uploadImageData];
	//[[FBRequest requestWithDelegate:self] call:@"facebook.photos.upload" params:nil dataParam:(NSData*)screenshot];
}
